      ...
      <div className="py-5 p-2 md:p-10">
        <div>
            ...
            <div>
              <label>
                ...
                <select
                  ...
                  name="table-lenght"
                  id="table-lenght"
                  className="border-1 bg-white border-black/10 p-2! py-1! outline-none! rounded-sm m-1 cursor-pointer w-auto!"
                >
            ...
            <div>
              <label>
                ...
                <input
                  ...
                  className="bg-white rounded-sm border-dashed border-1 border-black/10 focus:border-1! h-10 p-2 mr-3 focus:border-cupurple focus:outline-none!"
                />
              </label>
            </div>
          </div>
                  ...
                  {currentItems.map((item) => (
                    <tr key={item.id}>
                      <td className="py-4 relative flex ">
                        ...
                        <div className="overlay-edit absolute w-full h-full top-0 right-0 bg-[#FCFBFF] flex justify-center items-center gap-3">
                          ...
                          <FaRegTrashAlt
                            ...
                            className="w-10 h-10 bg-greenDark text-red-600 hover:text-white p-3 rounded-sm cursor-pointer"
                          />
                        </div>
                      </td>
                      <td>{item.position}</td>
                      ...
                      <td>
                        {item.status === "active" ? (
                          <>
                            <span className="bg-greenLight ttext-md rounded-sm text-greenDark px-2 py-1">
                              فعال
                            </span>
                          </>
                        ) : item.status === "inactive" ? (
                          <>
                            <span className="bg-red-200 ttext-md rounded-sm text-red-500 px-2 py-1">
                              غیر فعال
                            </span>
                          </>
                        ) : (
                          <>
                            <span className="bg-amber-100 ttext-md rounded-sm text-amber-500 px-2 py-1">
                              در انتظار تایید
                            </span>
                          </>
                        )}
          ...
          <div className="flex justify-between">
            <div>
              نمایش <span>{indexOfFirstItem + 1}</span> تا{" "}
              <span>{indexOfLastItem}</span> از{" "}
              <span>{filteredData.length}</span> رکورد
            </div>
            <div>
              <button
                className="p-2 border-1 border-black/10 rounded-tr-sm rounded-br- bg-white"
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                (page) => (
                  <button
                    className={`p-2 border-1 border-black/10 px-4 bg-white ${
                      currentPage === page ? "bg-cupurple! text-white " : ""
                    }`}
                    key={page}
              ...
              <button
                className="p-2 border-1 border-black/10 rounded-tl-sm rounded-bl-sm bg-white"
