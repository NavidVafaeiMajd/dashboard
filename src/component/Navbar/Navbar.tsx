        ...
        <> 
            ...
            <div id="navbar-content" className="lg:h-[calc(100vh-110px)]! h-full z-10 fixed w-[70%] mt-1 lg:w-[20%] lg:right-10 overflow-auto bg-navbar shadow-xl" style={{ direction: 'ltr', textAlign: 'right' }}>
                <ul>
                    ...
                    <NavbarItems DropMenu={false} itemInfo={[<> <AiOutlineMobile className='w-7 h-7'/> عملیات حقوق</>, "/hoghoogh"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <FiUserCheck className='w-7 h-7'/> مدیریت مشتریان</>, "/moshtarian"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <FiUserPlus className='w-7 h-7'/> رهبران </>, "/rahbaran"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <RiCameraLensLine className='w-7 h-7'/> ارزیابی عملکرد کارکنان </>, "/arzyabi"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <TbArrowsExchange2 className='w-7 h-7'/> ترفیعات و تبدیل وضعیت </>, "/tarfiyat"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <CiCircleQuestion className='w-7 h-7'/> پشتیبـانی </>, "/support"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <CiSquarePlus className='w-7 h-7'/> درخواست مرخصی </>, "/morkhasi"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <MdOutlineRadar className='w-7 h-7'/> آموزش </>, "/amoozesh"]} dropInfo={[]} />
                    <NavbarItems DropMenu={false} itemInfo={[<> <BsExclamationCircle className='w-7 h-7'/> پرونده های انضباطی </>, "/enzebati"]} dropInfo={[]} />
